...
import { completeLesson, INITIAL_PROGRESS_STATE, openChest } from '@/lib/progress';
...
type ProgressContextValue = {
  ...
  resetProgress: () => void;
  openChestById: (chestId: string) => void;
};
...
export function ProgressProvider({ children }: PropsWithChildren) {
  ...
  const value = useMemo<ProgressContextValue>(
    () => ({
      ...
      openChestById: (chestId: string) => {
        setProgress((prev) => {
          const next = openChest(prev, chestId);
