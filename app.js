...
function renderPdfPreview(){const f=readForm();const p=$('pdfPreview');p.innerHTML=`<div class="pdf-card"><h3>${f.planTitle||'課題プラン'}</h3><p>顧客名: ${f.customerName||'未入力'}</p><p>期間: ${f.startDate||'-'} 〜 ${f.endDate||'-'}</p><p>次回PT: ${f.nextTrainingDate||'未設定'}</p><p><strong>次回までの目標:</strong> ${f.nextGoal||'未入力'}（${f.goalPriority}）</p><p>${f.goalMemo||''}</p><p><strong>トレーナーからのメッセージ:</strong> ${f.trainerMessage||''}</p></div>`;
