...
function renderPdfPreview(){const f=readForm();const p=$('pdfPreview');p.innerHTML=`<div class="pdf-card pdf-goal-card"><h3>【次回までの目標】</h3><p>${f.nextGoal||'未入力'}</p><p><strong>優先度:</strong> ${f.goalPriority||'中'}</p><p><strong>目標メモ:</strong> ${f.goalMemo||'なし'}</p></div><div class="pdf-card"><h3>${f.planTitle||'課題プラン'}</h3><p>顧客名: ${f.customerName||'未入力'}</p><p>期間: ${f.startDate||'-'} 〜 ${f.endDate||'-'}</p><p>次回PT: ${f.nextTrainingDate||'未設定'}</p><p><strong>トレーナーからのメッセージ:</strong> ${f.trainerMessage||''}</p></div>`;
