...
function renderPdfPreview(){const f=readForm();const p=$('pdfPreview');const trainerMessage=(f.trainerMessage||'').trim();const trainerCard=trainerMessage?`<div class="pdf-card pdf-message-card"><h3>【トレーナーからのメッセージ】</h3><p>${trainerMessage.replace(/\n/g,'<br>')}</p></div>`:'';const locaboCard=`<div class="pdf-card pdf-rule-card"><h3>【ロカボルール】</h3><ul><li>1食の糖質量を20〜40gにすること</li><li>1日のトータル糖質量を120g以下にすること</li><li>タンパク質と脂質をしっかりとること</li></ul></div>`;p.innerHTML=`<div class="pdf-card pdf-title-card"><h3>${f.planTitle||'課題プラン'}</h3><p>顧客名: ${f.customerName||'未入力'}</p><p>期間: ${f.startDate||'-'} 〜 ${f.endDate||'-'}</p><p>次回PT: ${f.nextTrainingDate||'未設定'}</p></div>${trainerCard}${locaboCard}<div class="pdf-card pdf-goal-card"><h3>【次回までの目標】</h3><p>${f.nextGoal||'未入力'}</p><p><strong>優先度:</strong> ${f.goalPriority||'中'}</p><p><strong>目標メモ:</strong> ${f.goalMemo||'なし'}</p></div>`;
...
function downloadPdf(){const w=window.open('','_blank');w.document.write(`<html><head><title>PDF</title><style>body{font-family:sans-serif;padding:16px}.pdf-card{border:1px solid #ccc;border-radius:8px;padding:10px;margin-bottom:10px}</style></head><body>${$('pdfPreview').innerHTML}</body></html>`);w.document.close();w.focus();w.print();}
