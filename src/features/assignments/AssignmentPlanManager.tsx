...
type RestDaySuggestionOptions = {
  nextTrainingDate: string;
  personalTrainingDate: string;
};

const includesAnyText = (value: string, keywords: string[]) =>
  keywords.some((keyword) => value.toLowerCase().includes(keyword.toLowerCase()));

const needsExtraRecovery = (customer: Customer) =>
  customer.injuries.trim().length > 0 ||
  includesAnyText(`${customer.trainingHistory} ${customer.exerciseFrequency} ${customer.currentChallenges}`, [
    "初心者",
    "未経験",
    "なし",
    "痛",
...
const findDayIndexByDate = (days: AssignmentDayFormValues[], date: string) =>
  days.findIndex((day) => day.date === date);

const suggestRestDayIndexes = (
  days: AssignmentDayFormValues[],
  customer: Customer,
  { nextTrainingDate, personalTrainingDate }: RestDaySuggestionOptions,
) => {
  ...
  const recoveryInterval = needsExtraRecovery(customer) ? 3 : 5;

  if (dayCount >= 6) {
    restIndexes.add(Math.min(dayCount - 2, Math.max(1, Math.floor(dayCount / 2))));
  }

  for (let index = recoveryInterval - 1; index < dayCount - 1; index += recoveryInterval) {
    restIndexes.add(index);
  }

  const preNextTrainingDate = nextTrainingDate ? addDaysToDateInputValue(nextTrainingDate, -1) : "";
  const preNextTrainingIndex = preNextTrainingDate ? findDayIndexByDate(days, preNextTrainingDate) : -1;

  if (preNextTrainingIndex > 0) {
    restIndexes.add(preNextTrainingIndex);
  }

  const afterTrainingDate = personalTrainingDate ? addDaysToDateInputValue(personalTrainingDate, 1) : "";
  const afterTrainingIndex = afterTrainingDate ? findDayIndexByDate(days, afterTrainingDate) : -1;

  if (afterTrainingIndex >= 0) {
    restIndexes.add(afterTrainingIndex);
  }

  return restIndexes;
};

const applySuggestedRestDays = (
  days: AssignmentDayFormValues[],
  customer: Customer,
  options: RestDaySuggestionOptions,
): AssignmentDayFormValues[] => {
  const restIndexes = suggestRestDayIndexes(days, customer, options);

  return days.map((day, index) => {
    if (day.isRestDay || day.location === "rest" || restIndexes.has(index)) {
      return {
        ...day,
        location: "rest",
        isRestDay: true,
        trainingTasksText: "",
        cardioTask: day.cardioTask || "余裕があれば軽い散歩10分。疲れている場合は休む",
        mobilityTask: day.mobilityTask || "首・肩・股関節まわりを軽く5分ほぐす",
        sleepTask: day.sleepTask || "睡眠時間を確保し、疲労回復を優先する",
        memo: day.memo || "休養日です。痛みが出たら中止し、不安があればトレーナーに相談してください。",
        checkItemsText: day.checkItemsText || ["休養・ケア完了", "体調メモ", "睡眠を確認"].join("\n"),
      };
    }
...
export function AssignmentPlanManager() {
  ...
  const parseSafeDayCount = (value: string) => {
  ...
  const updateGeneratedDays = (nextStartDate: string, nextDayCount: number) => {
    setValue("days", generateAssignmentDayFormValues(nextStartDate, nextDayCount, getValues("days")), {
      shouldDirty: true,
      shouldValidate: true,
    });
  };
  ...
  const handleStartDateChange = (nextStartDate: string) => {
    const safeDayCount = parseSafeDayCount(getValues("dayCount")) ?? 1;

    setValue("startDate", nextStartDate, { shouldDirty: true, shouldValidate: true });
    setValue("endDate", addDaysToDateInputValue(nextStartDate, safeDayCount - 1), {
      shouldDirty: true,
      shouldValidate: true,
    });
    updateGeneratedDays(nextStartDate, safeDayCount);
  };

  const handleEndDateChange = (nextEndDate: string) => {
    const currentStartDate = getValues("startDate");
    const calculatedDayCount = calculateInclusiveDateCount(currentStartDate, nextEndDate);

    setValue("endDate", nextEndDate, { shouldDirty: true, shouldValidate: true });

    if (calculatedDayCount <= 0 || calculatedDayCount > 31) {
      return;
    }

    setValue("dayCount", String(calculatedDayCount), { shouldDirty: true, shouldValidate: true });
    updateGeneratedDays(currentStartDate, calculatedDayCount);
  };

  const handleDayCountChange = (nextDayCountValue: string) => {
    const parsedDayCount = parseSafeDayCount(nextDayCountValue);
    const currentStartDate = getValues("startDate");

    setValue("dayCount", nextDayCountValue, { shouldDirty: true, shouldValidate: true });

    if (!currentStartDate || parsedDayCount === null) {
      return;
    }

    setValue("endDate", addDaysToDateInputValue(currentStartDate, parsedDayCount - 1), {
      shouldDirty: true,
      shouldValidate: true,
    });
    updateGeneratedDays(currentStartDate, parsedDayCount);
  };
  ...
  const handleRegenerateDays = () => {
    ...
    setValue("endDate", addDaysToDateInputValue(getValues("startDate"), parsedDayCount - 1), {
      shouldDirty: true,
      shouldValidate: true,
    });
    updateGeneratedDays(getValues("startDate"), parsedDayCount);
  ...
  const adjustDayCount = (difference: number) => {
    ...
    updateGeneratedDays(getValues("startDate"), safeDayCount);
  };
  ...
  const handleGenerateMockAiAssignments = () => {
    ...
    const daysWithRestSuggestions = applySuggestedRestDays(currentDays, currentCustomer, {
      nextTrainingDate: getValues("nextTrainingDate"),
      personalTrainingDate: getValues("personalTrainingDate"),
    });

    setValue("days", generateMockAssignmentDays({ customer: currentCustomer, days: daysWithRestSuggestions }), {
      shouldDirty: true,
      shouldValidate: true,
    });
    setStatusMessage("休養日を自動設定し、モックAIで課題案を作成しました。内容は手動で修正できます。");
  };
  ...
  const handleGenerateAiAssignments = async () => {
    ...
    const requestBody: AiAssignmentGenerationRequest = {
      ...
      assignmentPeriod: {
        ...
        dayCount: daysWithRestSuggestions.length,
      },
      days: daysWithRestSuggestions,
      studyRule,
    };
    ...
    try {
      ...
      setStatusMessage(
        parsedResponse.data.source === "mock"
          ? "OPENAI_API_KEYが未設定のため、休養日を自動設定してモックAIで課題案を作成しました。内容を確認し、必要に応じて手直ししてください。"
          : "休養日を自動設定し、OpenAI APIで課題案を作成しました。内容を確認し、必要に応じて手直ししてください。",
      );
    } catch (error) {
      console.error(error);
      setStatusMessage(
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div className="mt-6 grid gap-4 md:grid-cols-2">
              ...
              <label className="flex flex-col gap-2">
                ...
                <input
                  type="date"
                  {...register("startDate", { onChange: (event) => handleStartDateChange(event.target.value) })}
                  className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-blue-400 focus:ring-4 focus:ring-blue-100"
                />
              ...
              <label className="flex flex-col gap-2">
                ...
                <input
                  type="date"
                  {...register("endDate", { onChange: (event) => handleEndDateChange(event.target.value) })}
                  className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-blue-400 focus:ring-4 focus:ring-blue-100"
                />
              ...
              <label className="flex flex-col gap-2">
                ...
                <div className="flex gap-2">
                  ...
                  <input
                    inputMode="numeric"
                    {...register("dayCount", { onChange: (event) => handleDayCountChange(event.target.value) })}
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-5 grid gap-4 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]">
                      <label className={`flex flex-col gap-2 ${isRestDay ? "opacity-70" : ""}`}>
                        <div className="flex items-center justify-between gap-2">
                          <span className="text-sm font-semibold text-slate-700">トレーニング課題（複数行）</span>
                          <TemplateApplySelect
                            label="トレーニング課題"
                            templates={templates}
                            categories={["home_training", "gym_training", "mobility"]}
                        ...
                        {isRestDay ? (
                          <span className="text-xs font-semibold text-amber-700">
                            休養日は軽い散歩・ストレッチなど回復中心の内容にしてください。
                          </span>
                        ) : null}
                      </label>
